...
import cc.dt.hrweb.common.annotation.Limit;
import cc.dt.hrweb.common.authentication.JWTToken;
import cc.dt.hrweb.common.authentication.JWTUtil;
import cc.dt.hrweb.common.domain.ActiveUser;
...
import cc.dt.hrweb.common.properties.HrWebProperties;
import cc.dt.hrweb.common.service.RedisService;
import cc.dt.hrweb.common.utils.*;
import cc.dt.hrweb.system.dao.LoginLogMapper;
import cc.dt.hrweb.system.domain.LoginLog;
...
import cc.dt.hrweb.system.service.LoginLogService;
...
public class LoginController {
    ...
    @Autowired
    private HrWebProperties properties;
    @Autowired
    private ObjectMapper mapper;

    @PostMapping("/login")
    @Limit(key = "login", period = 60, count = 20, name = "登录接口", prefix = "limit")
    public HrWebResponse login(
        ...
        if (User.STATUS_LOCK.equals(user.getStatus()))
            throw new HrwebException("账号已被锁定,请联系管理员！");
        ...
        return new HrWebResponse().message("认证成功").data(userInfo);
    }

    @GetMapping("index/{username}")
    public HrWebResponse index(@NotBlank(message = "{required}") @PathVariable String username) {
    ...
    @GetMapping("online")
    public HrWebResponse userOnline(String username) throws Exception {
        String now = DateUtil.formatFullTime(LocalDateTime.now());
        Set<String> userOnlineStringSet = redisService.zrangeByScore(HrWebConstant.ACTIVE_USERS_ZSET_PREFIX, now, "+inf");
        ...
        return new HrWebResponse().data(activeUsers);
    }
    ...
    public void kickout(@NotBlank(message = "{required}") @PathVariable String id) throws Exception {
        ...
        if (kickoutUser != null && StringUtils.isNotBlank(kickoutUserString)) {
            // 删除 zset中的记录
            redisService.zrem(HrWebConstant.ACTIVE_USERS_ZSET_PREFIX, kickoutUserString);
            // 删除对应的 token缓存
            redisService.del(HrWebConstant.TOKEN_CACHE_PREFIX + kickoutUser.getToken() + "." + kickoutUser.getIp());
        }
    }
    ...
    private String saveTokenToRedis(User user, JWTToken token, HttpServletRequest request) throws Exception {
        ...
        this.redisService.zadd(HrWebConstant.ACTIVE_USERS_ZSET_PREFIX, Double.valueOf(token.getExipreAt()), mapper.writeValueAsString(activeUser));
        // redis 中存储这个加密 token，key = 前缀 + 加密token + .ip
        this.redisService.set(HrWebConstant.TOKEN_CACHE_PREFIX + token.getToken() + StringPool.DOT + ip, token.getToken(), properties.getShiro().getJwtTimeOut() * 1000);
